...
請根據以上數據回答使用者問題。規範：
1. **完整回答清單**：若使用者詢問工單、機種等清單項目，請務必完整列出數據中的所有內容，不可擅自省略。
2. **排行與數據表格化**：排行或數值對比請使用 Markdown 網格表格呈現，確保排版美觀。
3. **結構化專業分析**：請針對數據進行有意義的解讀。除了列出事實外，應主動點出數據中的**異常點、最高/最低表現**，並根據數值給出專業的趨勢說明或解釋。不要侷限於句數，以說清楚講明白為準。
4. **禁止客服式廢話**：絕對不要出現「我是助手...」、「希望能幫到您」、「如果您還有問題...」等客套廢話。重點名稱或數據請使用 **粗體** 標示。"""
                    }
                ]
