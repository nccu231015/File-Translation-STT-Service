...
5. 詢問「哪些設備停機時間異常過長、停機確 Top-N、停機主因」 → 調用 `get_downtime_anomaly_ranking`
   ...
   - **僅問停機時間**（如「哪些設備停最久」「停機 Top-10」）：`include_cause=False`，回傳欄位為 `排名`、`樓層`、`設備(代碼)`、`停機時數(h)`
   - **同時問原因**（如「停機主因是什麼」「為什麼停機這麼多」）：`include_cause=True`，額外回傳 `主要停機原因`（由 B-code 與 DOWN 事件同秒共現推斷）
