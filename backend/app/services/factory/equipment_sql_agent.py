...
5. 詢問「哪些設備停機時間異常過長、停機確 Top-N、停機主因」 → 調用 `get_downtime_anomaly_ranking`
   ...
   - 回傳結果包含：排名、樓層位置、停機時數(h)（只計 DOWN 代碼 A001/A006-A009）
...
6. 詢問「比較兩個時期的停機原因/故障趨勢是否改善」等**使用者明確要求對比兩個不同期間**的查詢（如「本季 vs 上季」「上半年對下半年」）→ 調用 `get_fault_pattern_comparison`
   ...
   - 可鎖定設備、樓層或全廠
   - 回傳 `comparison`，用表格呈現兩期間總時數與變化趨勢
...
- 若詢問「設備概況/EQ-A」：表格只需顯示 `樓層`、`機型`、`設備(代碼)`、`RUN(分)`、`DOWN(分)`、`稼動率(%)`、`良率(%)`。稼動率 = RUN / (RUN + DOWN)。
...
- 若詢問「停機排名/EQ-E」：表格顯示 `排名`、`樓層`、`設備(代碼)`、`停機時數(h)`、`主要停機原因`。停機時數只計 DOWN 代碼（A001/A006-A009）；主要停機原因來自 CIM_MQTTCODEERR.NOTE（有5為該期間最常發生的故障說明）。
- 若詢問「故障分布比較/EQ-F」：使用 `comparison` 建立「停機時數對比表」，欄位為 `代碼`、`{{期間A}}停機(h)`、`{{期間B}}停機(h)`、`變化(h)`、`趨勢`；代碼定義請使用者自行確認，不要自行翻譯。趨勢欄位保留 ⬆ 惡化 / ⬇ 改善 / ─ 持平標記。
