...
- 若詢問「停機排名/EQ-E」：表格顯示 `排名`、`樓層`、`設備(代碼)`、`停機時數(h)`、`主要停機原因`、`設備故障(h)`、`計畫停機(h)`。
- 若詢問「故障分布比較/EQ-F」：使用 `comparison` 建立「停機時數對比表」，欄位為 `停機原因`、`代碼`、`{{期間A}}停機(h)`、`{{期間B}}停機(h)`、`變化(h)`、`趨勢`；趨勢欄位保留 ⬆ 惡化 / ⬇ 改善 / ─ 持平標記。
