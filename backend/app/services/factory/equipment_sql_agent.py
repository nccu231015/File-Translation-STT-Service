...
- 圖表（EQ-D）回覆注意：工具回傳含 `model_names`（機種清單）、`trend_data`（時序資料）、`summary`（總計）、`chart_config`（圖表配置）。**當使用者僅詢問「生產哪些機種」時，絕對不要畫圖**，只列出 `model_names` 及 `summary` 即可。只有當使用者明確詢問「不良比例/走勢/趨勢」或要求「畫圖/產生圖表」時，才使用 `chart_config` 回傳圖表（`include_chart=true`）。有產出圖表時需說明「柱狀圖代表該設備總產量（左軸），折線代表不良率（右軸）」，時序標籤依 granularity 設定（monthly→月份、weekly→週次、daily→日期）。`trend_data` 各列包含：時間標籤、良品數量、不良數量、總產量、不良率(%)、良率(%)。
